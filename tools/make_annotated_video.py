...
def read_events(path):
    ...
    with open(path, newline="") as f:
        ...
        for row in reader:
            ...
            # The ROS 2 bridge latency recorder uses a compact schema rather
            # than ORB-SLAM3's native EventLogger schema.  Normalise its
            # per-frame `track` rows so this replay tool can annotate either.
            if event == "track" and row.get("tracking_state_name"):
                state_name = row.get("tracking_state_name", "UNKNOWN")
                event_by_state = {
                    "OK": "TRACKING_STABLE",
                    "RECENTLY_LOST": "TRACKING_RECENTLY_LOST",
                    "LOST": "TRACKING_LOST",
                    "NOT_INITIALIZED": "TRACKING_NOT_INITIALIZED",
                    "NO_IMAGES_YET": "TRACKING_NO_IMAGES_YET",
                }
                normalised = dict(row)
                normalised.update({
                    "module": "Tracking",
                    "event": event_by_state.get(state_name, f"TRACKING_{state_name}"),
                    "state_name": state_name,
                    "matches_inliers": row.get("tracking_inliers", "N/A"),
                    "current_kf": row.get("reference_keyframe_id", "N/A"),
                    "details": (
                        f"map={row.get('map_id', 'N/A')} "
                        f"TrackStereo={row.get('track_stereo_ms', 'N/A')} ms"
                    ),
                })
                tracking_by_frame[frame_id] = normalised
                continue

            # Normalise discrete bridge events for display in the Events list.
            if not module and event:
                row = dict(row)
                row["module"] = "ORBBridge"
                row["current_kf"] = row.get("reference_keyframe_id", "N/A")
                row["details"] = (
                    f"state={row.get('tracking_state_name', 'N/A')} "
                    f"map={row.get('map_id', 'N/A')}"
                )
                module = row["module"]
...
def choose_status_color(event):
    if event == "HUMANSLAM RELOCALISATION SUCCESS":
        return (80, 255, 80)
...
def main():
    ...
    parser.add_argument(
        "--semantic_debug_dir", default="",
        help="Optional HumanSLAM overlay frames named with six-digit frame IDs",
    )
    parser.add_argument(
        "--humanslam_success_frames", default="",
        help="Comma-separated frames confirmed by ORB as semantic relocalisations",
    )
    ...
    humanslam_success_frames = {
        int(value) for value in args.humanslam_success_frames.split(",")
        if value.strip()
    }
    ...
    for frame_idx in range(args.start_frame, n, args.step):
        semantic_path = (
            Path(args.semantic_debug_dir) / f"{frame_idx:06d}.png"
            if args.semantic_debug_dir else None
        )
        image_path = (
            semantic_path
            if semantic_path is not None and semantic_path.exists()
            else images[frame_idx]
        )
        ...
        if frame_idx in humanslam_success_frames:
            special_events = list(special_events) + [{
                "module": "HumanSLAM",
                "event": "HUMANSLAM_RELOCALISATION_SUCCESS",
                "current_kf": "N/A",
                "matched_kf": "see candidate CSV",
                "details": "ORB geometric verification accepted a semantic candidate",
            }]
        ...
        if frame_idx in humanslam_success_frames:
            status_event = "HUMANSLAM RELOCALISATION SUCCESS"
